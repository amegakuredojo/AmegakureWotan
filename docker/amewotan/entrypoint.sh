...
# ── 1. BOOT INFO ─────────────────────────────────────────────────────────────
echo "[BOOT] Starting AmegakureWotan Core with Kùzu embedded GraphDB..."
...
BYPASS_TOR="${AMEWOTAN_OPSEC_BYPASS_TOR:-false}"

if [ "$BYPASS_TOR" = "true" ]; then
    echo "[BOOT] AMEWOTAN_OPSEC_BYPASS_TOR=true — Modo Sandbox/Dev: Tor check OMITIDO."
...
echo "[BOOT] Initializing AmegakureWotan data directories..."
python3 -c "
...
ledger.log_execution(
    ...
    parameters={
        ...
        'kuzu_db_path': os.environ.get('KUZU_DATABASE_PATH', '/data/amegakurewotan_vault.kuzu'),
        'opsec_bypass_tor': os.environ.get('AMEWOTAN_OPSEC_BYPASS_TOR', 'false'),
...
echo ""
echo "[BOOT] ══════════════════════════════════════════════"
echo "[BOOT] AmegakureWotan is OPERATIONAL. All systems GO."
echo "[BOOT] ══════════════════════════════════════════════"
echo ""

# ── 7. EJECUTAR COMANDO SOLICITADO ───────────────────────────────────────────
if [ "$#" -eq 0 ]; then
    exec python3 -m amegakurewotan.cli tui
elif [ "$1" = "pytest" ] || [ "$1" = "bash" ] || [ "$1" = "sh" ] || [ "$1" = "ls" ] || [ "$1" = "python" ] || [ "$1" = "python3" ] || [ "$1" = "amewotan" ] || [ "$1" = "/usr/local/bin/amewotan" ]; then
    exec "$@"
else
    if [ "$IS_MCP" = "true" ]; then
        # Restaurar stdout original (fd3) para que MCP pueda escribir JSON-RPC
        exec python3 -m amegakurewotan.cli "$@" >&3
    else
        exec python3 -m amegakurewotan.cli "$@"
    fi
fi
