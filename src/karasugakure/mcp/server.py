...
"""
Módulo: karasugakure.mcp.server
Contexto: CIVIL
...
Prerequisitos: Kùzu DB, SearXNG, Tor proxy (opcional — configurable por KARASU_OPSEC_BYPASS_TOR)
...
def get_kuzu_connection() -> kuzu.Connection:
    ...
    if _conn is None:
        try:
            db_path = os.environ.get("KUZU_DATABASE_PATH", "/data/karasu_vault.kuzu")
...
async def call_tool(name: str, arguments: dict) -> List[TextContent]:
    ...
    # ── Tool: hel_darkweb ────────────────────────────────────────────────────
        ...
        try:
            ...
            from karasugakure.policy.opsec import OPSECViolationException
            agent = HelAgent()
