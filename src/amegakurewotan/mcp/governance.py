...
"""
Módulo: amegakurewotan.mcp.governance
Contexto: CIVIL — Consolidación AmegakureWotan (capa L0, gobernanza MCP F5)

Propósito:
    Funciones de gobernanza Wotan (F5) para el servidor MCP (AmegakureWotanMCP).
    ...
    MCP lo consume vía `from amegakurewotan.mcp.governance import govern, seal_execution`.
"""
...
def govern(name: str, arguments: Dict[str, Any]) -> Tuple[str, Optional[str]]:
    ...
    from amegakurewotan.policy.gelsi import ActionRequest, Decision, get_gelsi
    ...
    verdict = get_gelsi().evaluate(
        ActionRequest(
            ...
            roe_token=roe_token,
            intent=name,
            collector_id="mcp:AmegakureWotanMCP",
