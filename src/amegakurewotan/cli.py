...
from amegakurewotan.graph.export import export_to_json, export_all_nodes
from amegakurewotan.agents.odin import OdinAgent
from amegakurewotan.agents.norn import NornAgent
...
from amegakurewotan.agents.mimir import MimirAgent
...
app = typer.Typer(help="AmegakureWotan: CLI-only OSINT Orchestration Harness")
...
# ── AmegakureWotan: capa consolidada (GELSI/RoE/MCP/DFIR) montada bajo 'amegakurewotan wotan' ──
from amegakurewotan.cli_wotan import app as wotan_app
...
def run(
    ...
    """
    Start the AmegakureWotan Engine. 
    By default runs the MCP Server (stdio) for Claude/Cursor.
    Use --autonomous to run the offline CodeAgent.
    """
    if autonomous:
        console.print("[bold green]✔[/bold green] Iniciando AmegakureWotan en modo AUTÓNOMO (Sub-agentes locales)...")
        from amegakurewotan.agents.code_agent import create_osint_code_agent
        ...
        import asyncio
        from amegakurewotan.mcp.server import main as mcp_main
...
def init():
    ...
    console.print("[bold green]✔[/bold green] AmegakureWotan directories initialized at [cyan]~/.amegakurewotan[/cyan]")
...
def report(
    ...
    console.print(Panel(
        ...
        "Status: CONFIDENTIAL\n"
        "Generated using AmegakureWotan Engine v0.1.0\n\n"
        "Use `amewotan export` to generate a full JSON output.",
        title="Dossier Draft"
    ))
...
@app.command()
def tui():
    """Launch the interactive AmegakureWotan Terminal User Interface."""
    from amegakurewotan.tui import KarasuTuiApp
