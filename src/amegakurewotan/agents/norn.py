        ...
        if query in ["hello", "hi", "test", "help", "query", "search", "what is this", "run", "go", "dump", "view"]:
            raise ValueError(
                json.dumps({
                    "status": "failure",
                    "error": "VagueQuery",
                    "message": "Query is too vague. AmegakureWotan requires deterministic OSINT graph intent specifying nodes or actions.",
        ...
        if re.search(r"\b(why|how|should|recommend|describe|summarize|analyze|who|help)\b", query):
            raise ValueError(
                json.dumps({
                    ...
                    "message": "Query is too ambiguous or conversational. AmegakureWotan requires deterministic OSINT graph intent.",
                    "original_query": natural_language
                }, indent=2)
            )
